...
@if(isset($getCompany->region_id) && !empty($getCompany->region_id))
<script type="text/javascript">
    $.ajax({
    ...
    setTimeout(function(){$('#comuna_id').val({{$getCompany->comuna_id}}).change(); }, 1000);
</script>
@endif
        ...
        $("#addmore").on("click",function(){
            var rowCount = $('#sortable >tbody >tr').length;
               ...
               $.ajax({
                ...
                data:{'action':'addDataRow',"_token": "{{ csrf_token() }}",'counter':rowCount},
