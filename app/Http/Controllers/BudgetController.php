...
class BudgetController extends Controller
{
    ...
    public function accountList(Request $request)
    {
        if(!empty($_POST['action']) && $_POST['action'] == 'listRecords') { 
            ...
            ->addColumn('action', function($row){
                $btn = '<button type="button" name="update" id="'.$row->account_id.'" class="btn btn-primary ri-edit-2-line update"> Edit</button>'; 
                return $btn;
            })
            ->rawColumns(['action'])
            ->make(true); 
    ...
    public function costCentreList(Request $request)
    {
        if(!empty($_POST['action']) && $_POST['action'] == 'listRecords') { 
            ...
            ->addColumn('action', function($row){
                $btn = '<button type="button" name="update" id="'.$row->cost_centre_id.'" class="btn btn-primary ri-edit-2-line update"> Edit</button>'; 
                return $btn;
            })
            ->rawColumns(['action'])
            ->make(true); 
    ...
    public function budgetsProjectList(Request $request)
    {
        if(!empty($_POST['action']) && $_POST['action'] == 'listRecords') { 
            ...
            ->addColumn('action', function($row){
                $btn = '<button type="button" name="update" id="'.$row->budget_id.'" class="btn btn-primary ri-edit-2-line update"> Edit</button>'; 
                return $btn;
            })
            ->rawColumns(['action'])
            ->make(true); 
    ...
    public function expenseTypeList(Request $request)
    {
        ...
        if(!empty($_POST['action']) && $_POST['action'] == 'listRecords') { 
            ...
            ->addColumn('action', function($row){
                $btn = '<button type="button" name="update" id="'.$row->expense_type_id.'" class="btn btn-primary ri-edit-2-line update"> Edit</button>'; 
                return $btn;
            })
            ->rawColumns(['action'])
            ->make(true); 
